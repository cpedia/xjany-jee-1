...
public class Tactical_Asset_Allocation_deal_with_profile0_pool_select_201005061102 extends SimulateStrategy{
	public Tactical_Asset_Allocation_deal_with_profile0_pool_select_201005061102(){
		super();
		StrategyID=1102L;
		StrategyClassID=3L;
	}
...
HashMap<String,List<String>> AllAssetFundsMap = new HashMap<String,List<String>>();     // modified on 20100506
HashMap<String,List<String>> AvailableAssetsMap = new HashMap<String,List<String>>();
if(RiskProfile != 0)
     AllAssetFundsMap = getAvailableAssetClassSet(MainAssetClassList, CandidateFundList);
else
{
    CandidateFundList.add("CASH");
    CandidateFundList.add("CASH");
    CandidateFundList.add("CASH");
    AllAssetFundsMap.put("ROOT", CandidateFundList);
    AvailableAssetsMap = getAvailableAssetClassSet(MainAssetClassList, CandidateFundList);
}
...
List<Double> SelectedAssetPercentages;
if(RiskProfile == 0)
{
SelectedAssets = RiskyGroupSelect;
...
{
    iterator = AvailableAssetsMap.keySet().iterator();
    while(iterator.hasNext()){
        String key = (String)iterator.next();
        if(key.equals("CASH"))    HaveCashAsset = true;  
        CurrentAsset=new Asset();
        CurrentAsset.setAssetStrategyID(getStrategyID("STATIC"));
        CurrentAsset.setName(key);
        CurrentAsset.setClassID(getAssetClassID(key));
...
for(int i =0; i < SelectedAssets.size(); i++)
{
    ...
    for(int j=0; j< RepresentFundList.size(); j++)
    {
       ...
       {
           BuyAsset = new Asset();
           BuyAsset = CurrentPortfolio.getAsset(getSecurity(RepresentFundList.get(j)).getAssetClass().getName());
           BuyAsset.setTargetPercentage(0.33);
           CurrentPortfolio.buyAtNextOpen(BuyAsset.getName(), RepresentFundList.get(j), TotalAmount*FundPercentageMap.get(RepresentFundList.get(j)).doubleValue(), CurrentDate,true);
       }
    }
}
	}
...
if(RiskProfile == 0)    
{
    MainAssetClass = new String[1];
    MainAssetClass[0] = "ROOT";
    MaxOfRiskyAsset  =  3;
    MaxOfStableAsset = 3;
}
