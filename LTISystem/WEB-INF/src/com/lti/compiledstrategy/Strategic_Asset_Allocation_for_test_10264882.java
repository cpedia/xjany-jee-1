...
public class Strategic_Asset_Allocation_for_test_10264882 extends SimulateStrategy{
	public Strategic_Asset_Allocation_for_test_10264882(){
		super();
		StrategyID=4882L;
		StrategyClassID=3L;
	}
...
@Override
  public void afterExecuting() throws Exception {
    ...
    com.lti.util.PersistentUtil.writeGlobalObject(_storeData, "SAA_Data_Plan_" + PlanID,StrategyID,PortfolioID);
    com.lti.util.PersistentUtil.writeGlobalObject(_holdingInfo, "HoldingInfoPlan_" + PlanID,StrategyID,PortfolioID);
  }
  ...
  public Map<String, Boolean> ExcludeByRoundTripAndWaiting(List<String> NeedTestFundList, Map<String, CandidateFundType> CandidateFundMap, boolean NoInfo, Map<String,Integer> roundtripInfo, Map<String,Integer> waitingPeriodInfo) throws Exception {
    ...
    for (int i = 0; i < NeedTestFundList.size(); i++) {
      ...
      else
      {
        ...
        {cft.setRoundtripLimit(CandidateFundMap.get(fund).getRoundtripLimit()); roundtripInfo.put(fund,cft.getRoundtripLimit()); printToLog("Add extra info into data object. " + fund);}
        if(waitingPeriodInfo.get(fund) != null)
            cft.setWaitingPeriod(waitingPeriodInfo.get(fund)); 
        else
        {cft.setWaitingPeriod(CandidateFundMap.get(fund).getWaitingPeriod()); waitingPeriodInfo.put(fund, cft.getWaitingPeriod()); printToLog("Add extra info into data object. "+ fund);}    
        NeedTestFundMap.put(fund,cft);
        printToLog(fund + "' Roundtrip limit = " + cft.getRoundtripLimit() + "  waiting period = " + cft.getWaitingPeriod());
      }
    }
