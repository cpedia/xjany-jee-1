...
public class Total_Return_Table611 extends SimulateStrategy{
	public Total_Return_Table611(){
		super();
		StrategyID=611L;
		StrategyClassID=3L;
	}
...
String[][] ReturnTable = new String[Funds.length + 1][8];
  ...
  /*************************** create indicator table****************************************************/
      if(DoLast){
            ...
            CurrentDate = LTIDate.clearHMSM(CurrentDate);
            Calendar ca = Calendar.getInstance();
            Calendar lca = Calendar.getInstance();
            ca.setTime(CurrentDate);          
            lca.setTime(ca.getTime());
            lca.add(Calendar.DAY_OF_YEAR, -1);
            while(lca.get(Calendar.DAY_OF_WEEK)!=caculateday+1){
            	lca.add(Calendar.DAY_OF_YEAR, -1);
            }
            while(!LTIDate.isNYSETradingDay(lca.getTime())){
	         lca.add(Calendar.DAY_OF_YEAR, -1);
           }
