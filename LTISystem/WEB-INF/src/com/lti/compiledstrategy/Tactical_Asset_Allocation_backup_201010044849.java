...
public class Tactical_Asset_Allocation_backup_201010044849 extends SimulateStrategy{
	public Tactical_Asset_Allocation_backup_201010044849(){
		super();
		StrategyID=4849L;
		StrategyClassID=3L;
	}
...
@Override
  public void afterExecuting()   throws Exception {
    ...
    com.lti.util.PersistentUtil.writeObject(_storeData, "TAA Data Plan " + PlanID);
  }
  @Override
...
public void initmpts(Map<String, CandidateFundType> CandidateFundMap)  throws Exception {
   ...
   * @return @
   */
  public Object[] SelectAssetClass(String[] AvailableAssets, String[] StableAssets, double CashScoreWeight, int SelectedPercentage, int _NumberOfMainRiskyClass, int _NumberOfMainStableClass, Map<String, CandidateFundType> CandidateFundMap)  throws Exception {
  ...
  public void initStoreData(String dateStr, Map<String, CandidateFundType> CandidateFundMap) throws Exception {
    if(_storeData==null ){
      try {
        _storeData = (Map<String, List<String>>) com.lti.util.PersistentUtil.readObject("TAA Data Plan " + PlanID);
      } catch (Exception e1) {
       }
    }
    ...
    // 初始化AvailableFundMap
    Map<String, CandidateFundType> AvailableFundMap = ChooseAvailableCandidate(CandidateFundMap);
    
   /*Iterator<String> it = AvailableFundMap.keySet().iterator();             //For testing
   while(it.hasNext()){
       String key = it.next();
       if(AvailableFundMap.get(key).isShortHistory())
           printToLog("Short History Fund : " + key);
   }*/
      ...
      // Calculate the momentum scores of benchmarks and select asset
      // classes
      Object[] SelectClassObject = SelectAssetClass(AvailableAssets, StableAssets, CashScoreWeight, SelectedPercentage, _NumberOfMainRiskyClass, _NumberOfMainStableClass, CandidateFundMap);
      ...
      for (int i = 0; i < SelectedAssets.size(); i++) {
        ...
        printToLog("Fund List of " + SelectedAssets.get(i) + " : " + FundList);
       ...
       // printScore(_Funds);
        ...
        for (int j = 0; j < SaveNumber; j++) {
          ...
          List<String> strs = _storeData.get(tmpkey);
          if (strs == null) {
  ...
  public void BuildSelectedAsset(Map<String, Double> SelectedAssetPercentagesMap, Map<String, List<String>> AssetRepresentFundMap, Map<String, Double> FundPercentageMap, Map<String, Boolean> CanNotBuyFundMap, String dateStr, Map<String, CandidateFundType> CandidateFundMap) throws Exception {
    ...
    List<String> tmpRiskyGroupSelect = _storeData.get(RISKY_GROUP + "." + dateStr);
    ...
    List<String> tmpStableGroupSelect = _storeData.get(STABLE_GROUP + "." + dateStr);
    ...
    while (iter.hasNext()) {// asset treatment
      ...
      {
         tempMap = ExcludeByRoundTripAndWaiting(SaveFundList, CandidateFundMap, _RoundtripLimit, _WaitingPeriod);
         if(tempMap.size() > 0)
      ...
      for (int j = 0; j < MaxIndex; j++) {
        if (FundPercentageMap.get(SaveFundList.get(j).trim()) == null) {
          if (CanNotBuyFundMap.get(SaveFundList.get(j)) == null || SimulateHolding.getHoldingItem(SaveFundList.get(j)) != null) // Adj
          // for
          // RoundTrip
          // Part
          // 3
          ...
          if (RepresentFundList.indexOf(SaveFundList.get(j)) == -1)
            RepresentFundList.add(SaveFundList.get(j));// e.g, add
          // "CASH"
          // fund
          // under
          // "CASH"
  ...
  public Map<String, Boolean> ExcludeByRoundTripAndWaiting(List<String> NeedTestFundList, Map<String, CandidateFundType> CandidateFundMap, int[] RoundtripLimit, int[] WaitingPeriod)  throws Exception {
    ...
    if (WaitingPeriod != null) {
      for (int ifund = 0; ifund < WaitingPeriod.length; ifund++) {
        if (WaitingPeriod[ifund] != 0) {
          NeedToCalWaitingPeriod = true;
          break;
        }
      }
    }// Default WaitingPeriod is zero month(immediately) before a fund is
    // bought back.
    ...
    if (NeedToCalRoundtrip) {
      for (int imonth = 12; imonth > 0; imonth--) {
        ...
        for (int ifund = 0; ifund < funds.length; ifund++) {
          CandidateFundType cft = CandidateFundMap.get(funds[ifund]);
          ...
          CandidateFundMap.put(funds[ifund],cft);
      ...
      /* LJF Temp Testing */printToLog("CanNotBuyFund due to RoundtripLimit: ", CanNotBuyFundMap);
    }
...
@SuppressWarnings("deprecation")
  public Object[] Adjust529SelectedAsset(Map<String, Double> SelectedAssetPercentagesMap, Map<String, List<String>> AssetRepresentFundMap, Map<String, Double> FundPercentageMap, String dateStr, Map<String, CandidateFundType> CandidateFundMap, int ToRebalance)  throws Exception {
    ...
    if (NeedToAdjNow) {
      ...
      // Transaction Type 2~~~
        ...
        // --for current holding risky asset and their funds(Add
        // KeepStill*ActualPercentage to NewFundPercentageMap)
        ...
        for (int iAsset = 0; iAsset < CurrentAssetList.size(); iAsset++) {
          ...
          if (!AssetName.equals("FIXED INCOME") && !AssetName.equals("INTERNATIONAL BONDS") && !AssetName.equals("CASH") && AssetAmount > 0) {
            ...
            /* LJF temp testing */
            if (SelectedAssetPercentagesMap.get(AssetName) != null)
              printToLog("LJF warning:An Error occured!!!TargetAssets contains risky Asset");
            NewSelectedAssetPercentagesMap.put(AssetName, AssetAmount / PortfolioTotalAmount * KeepStill);
          }
        }

        // --for target assets(Adjust
        // TargetAssetPercentages.get()*(1-SumHoldingFundPercent))
        {
          Iterator<String> iter = SelectedAssetPercentagesMap.keySet().iterator();
          while (iter.hasNext()) {
            String AssetName = iter.next();
            if (AssetName.equals("FIXED INCOME") || AssetName.equals("INTERNATIONAL BONDS") || AssetName.equals("CASH")) {
              /* LJF temp testing */
              if (NewSelectedAssetPercentagesMap.get(AssetName) != null)
                printToLog("LJF Important Warning: An error occured!!! This situation need more attention");
              NewSelectedAssetPercentagesMap.put(AssetName, SelectedAssetPercentagesMap.get(AssetName) * (1 - SumHoldingFundPercent));
            }
          }
        }
        // --for target Favorable funds (Add
        // TargetFundPercentageMap.get(FavSecurity)*(1-SumHoldingFundPercent)
        // to NewFundPercentageMap)
        {
          Iterator<String> iter = FundPercentageMap.keySet().iterator();
          while (iter.hasNext()) {
            String FundSymbol = iter.next();
            double PercentAmount = FundPercentageMap.get(FundSymbol);
...
//Adj for Roungtrip Part 0 LJF and adj for 529(actually Initial for getParameter Part)
    //private int[] _RoundtripLimit;
    //private int[] _WaitingPeriod;
    private boolean _is529Plan;
    private boolean _rewriteData;
    private void initPlanParameters(){
      ...
      if (tmp==null){_RoundtripLimit=null;_WaitingPeriod=null;_is529Plan=false;_rewriteData=false;return;}
...
// 初始化持久化变量
   ...
   int ToRebalance = 1;
    ...
    if (_is529Plan) {
      ...
      Object[] obj = Adjust529SelectedAsset(SelectedAssetPercentagesMap, AssetRepresentFundMap, FundPercentageMap, dateStr, CandidateFundMap, ToRebalance);
...
|| ( CheckFrequency.equalsIgnoreCase("quarterly") && LTIDate.isLastNYSETradingDayOfQuarter(LTIDate.getNewDate(CurrentDate, TimeUnit.DAILY, 31 - CheckDate))) || (CheckFrequency.equals("yearly")&&LTIDate.isLastNYSETradingDayOfYear(LTIDate.getNewDate(CurrentDate, TimeUnit.DAILY, 31 - CheckDate)))) {
			...
			/*
			 * Form Holding Map , calculate available tading amount of each
			 * security and each asset
			 */
