...
public class Tactical_Asset_Allocation_backup_201008264734 extends SimulateStrategy{
	public Tactical_Asset_Allocation_backup_201008264734(){
		super();
		StrategyID=4734L;
		StrategyClassID=3L;
	}
...
Map<String,String> AssetClassMap = new HashMap<String,String>();
for(int i = 0; i < CandidateFund.length; i++)
{
    try{AssetClass[i] = getSecurity(CandidateFund[i]).getAssetClass().getName(); }  
        catch(Exception e){AssetClass[i] = "Unknow";}
     AssetClassMap.put(CandidateFund[i], AssetClass[i]);
}
...
//try{
if((CheckFrequency.equals("monthly")&&LTIDate.isLastNYSETradingDayOfMonth(LTIDate.getNewDate(CurrentDate, TimeUnit.DAILY, (31- CheckDate)))) || (CheckFrequency.equals("quarterly")&&LTIDate.isLastNYSETradingDayOfQuarter(LTIDate.getNewDate(CurrentDate, TimeUnit.DAILY, (31- CheckDate)))) || 
(CheckFrequency.equals("yearly")&&LTIDate.isLastNYSETradingDayOfYear(LTIDate.getNewDate(CurrentDate, TimeUnit.DAILY, (31- CheckDate)))))
{
                Calendar tmpCa = Calendar.getInstance();
		tmpCa.setTime(CurrentDate);
...
/*LJF temp testing*/printToLog("CandidateFundList: "+CandidateFundList);
/*LJF temp testing*/printToLog("AllAssetFundsMap1:"+AllAssetFundsMap);
String[] AvailableAssets= new String[AllAssetFundsMap.size()];
int[] IndexOfStable = new int[AvailableAssets.length]; 
Iterator iterator = AllAssetFundsMap.keySet().iterator();
int n1 = 0; int n2 = 0;
...
/*LJF temp testing*/printToLog("AllAssetFundsMap2:"+AllAssetFundsMap);
...
/*LJF temp testing*/printToLog("startDate: "+startDate+", CurrentDate: "+CurrentDate+",iDuplicateFund: "+iDuplicateFund[0]+", TransType: "+TransType);
...
for(int i = 0; i < SelectedAssets.size(); i++)
{
    ...
    // Special Treatment towards ShortHistoryFunds   , Added by WYJ on 2010.05.13, modified on 2010.08.03
    for(int j = 0; j< FundList.size(); j++)
    {
        if(ShortHistoryFundList.indexOf(FundList.get(j)) == -1)
            Funds[j] = FundList.get(j);    
...
for(int j = 0; j < MaxIndex; j++)  
{
    if(!Funds[j].equals("CASH"))
    {
        if(RepresentFundList.indexOf(Funds[j]) == -1)
		{	
			if (CanNotBuyFundList.indexOf(Funds[j])== -1 || CurrentPortfolio.holdSecurity(getSecurity(Funds[j]).getID())) //Adj for RoundTrip Part 3 LJF 2010.07.02
			{
			RepresentFundList.add(Funds[j] );
            FundPercentageMap.put(Funds[j].trim(), Weight);
            printToLog(SelectedAssets.get(i) + " buy target :  " + Funds[j] + "   target percentage : " + Weight);
				...
				if (MaxIndex < Funds.length) {MaxIndex=MaxIndex+1;} 
...
/*LJF temp testing*/printToLog("AssetRepresentFundMap"+AssetRepresentFundMap);
	...
	for (int ifund=0;ifund<CanNotBuyFundList.size();ifund++)
	{
		...
		for (int NameIndex=0;NameIndex<MainAssetClassList.size();NameIndex++)
		{
	...
	for(int iasset=0;iasset<NeedTestAssetList.size();iasset++)
	{
		...
		{			
			...
			if(iAssetName > -1)
			{
				...
				for (int itemp=0;itemp<RiskyAssets.length;itemp++)
					{if (RiskyAssets[itemp].equals(AssetName)) {RiskyAssetIndex = itemp;break;} }
		...
		for (int ifund=0;ifund<RepresentFundList.size();ifund++)
		{
			...
			{
				if (CurrentPortfolio.getSecurityAmount(AssetName,FundName,CurrentDate)/CurrentPortfolio.getTotalAmount(CurrentDate) < FundPercentageMap.get(FundName)) // Detect for CanNotBuy Funds that may need for Buy Rebalance Action
				{
				...
				else {printToLog("LJF Important Notice: CurrentHolding has this CanNotBuyFund "+FundName+", but its actual percentage"+(CurrentPortfolio.getSecurityAmount(AssetName,FundName,CurrentDate)/CurrentPortfolio.getTotalAmount(CurrentDate))+" > Target percentage "+FundPercentageMap.get(FundName));}
			}
		}
	...
	for(int iasset=0;iasset<RiskyAssets.length;iasset++)
	{
		if (AssetRepresentFundMap.get(RiskyAssets[iasset])==null && CanNotAdjRiskyAssetList.indexOf(RiskyAssets[iasset])==-1) CanNotAdjRiskyAssetList.add(RiskyAssets[iasset]);
	}
	for(int iasset=0;iasset<StableAssets.length;iasset++)
	{
		if (AssetRepresentFundMap.get(StableAssets[iasset])==null && CanNotAdjStableAssetList.indexOf(StableAssets[iasset])==-1) CanNotAdjStableAssetList.add(StableAssets[iasset]);
	}
...
*/
	...
	if (UnAllocateRiskyPercentage > 0 && (RiskyAssets.length !=CanNotAdjRiskyAssetList.size()))
	{
		double AddOn=UnAllocateRiskyPercentage/(RiskyAssets.length-CanNotAdjRiskyAssetList.size());
		for (int iasset=0;iasset<RiskyAssets.length;iasset++)
		{
			String AssetName=RiskyAssets[iasset];
	...
	if (UnAllocateStablePercentage > 0 && (StableAssets.length != CanNotAdjStableAssetList.size()))
	{
		double AddOn=UnAllocateStablePercentage/(StableAssets.length-CanNotAdjStableAssetList.size());
		for (int iasset=0;iasset<StableAssets.length;iasset++)
		{
			String AssetName=StableAssets[iasset];
...
for(int i = 0; i < HoldingAssetList.size(); i++)
{
    ...
    {
        printToLog("adjusted available percentage = " + AssetAvailableTradingPercentage.get(HoldingAssetList.get(i).getName().trim()) );
        continue;
    }
    else
    {
...
TotalActualTradingPercentage = TotalTargetSellPercentage - UnableToSellPercentage;  //actual can sell percentage

printToLog("For debug:  Actual selling percentage (not  final) = "+  AssetActualTradingPercentage);


double BuyWeight = TotalActualTradingPercentage / TotalTargetSellPercentage;
printToLog("For debug : TotalActualTradingPercentage / TotalTargetSellPercentage   =   " + BuyWeight );
...
printToLog("For debug: Actual trading percentage (before deal with left percentage) = " +  AssetActualTradingPercentage);
...
printToLog("For debug:  Actual trading percentage (after deal with left percentage) = " +  AssetActualTradingPercentage);
...
for(int i = 0; i < SelectedAssets.size(); i++)
{
    ...
    if(TargetPercentage != ActualPercentage)  // added by WYJ on 2010.08.10, let the calculation take care of  the un-traded trading fraction of the former asset class 
    {
...
while(iter1.hasNext() && !Done)
{
    ...
    while(iter2.hasNext() && !Done)
    {
        ...
        if(TempPercentage * UnTradePercentage >0 )      // Modified on 2010.08.10, combine the fraction trading into the other trading with the same sign
        {
...
for(int i = 0; i < SelectedAssets.size(); i++)
{
    ...
    if(SecurityActualTradingPercentage != null)
    {
        ...
        if(TranNum > 1 && TradePercentage < 0){
          try{
...
///usr/apache-tomcat-6.0.16/webapps/LTISystem/WEB-INF/src/com/lti/compiledstrategy/Tactical_Asset_Allocation_backup_201008264734.java:582: incompatible types
