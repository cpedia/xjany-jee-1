...
public class LJF_Strategic_Asset_Allocation_with_RoundTrip_and_5291038 extends SimulateStrategy{
	public LJF_Strategic_Asset_Allocation_with_RoundTrip_and_5291038(){
		super();
		StrategyID=1038L;
		StrategyClassID=3L;
	}
...
for(int i =0; i< AvailableAssetClassList.size(); i++)
{
  ...
  else
  {
    ...
    if(SharpeMonths != 0)
    {
        ...
        if(AvailableAssetClassList.get(i).equals("US EQUITY"))  printToLog("US EQUITY ranking : " + newArray); /*For Testing*/
	int FundNumber = MaxOfEachAssetArray[i] > Funds.length ? Funds.length : MaxOfEachAssetArray[i] ;
	SelectedFunds = new ArrayList<String>();
	for(int j = 0; j< FundNumber; j++){ 	//To obtain SelectedFunds List      
...
for(int i =0; i< CandidateAsset.size(); i++){
	if(CandidateAsset.get(i).equals("FIXED INCOME") || CandidateAsset.get(i).equals("INTERNATIONAL BONDS") || CandidateAsset.get(i).equals("CASH")){
		A[i] = 2; 
		...
		private void initPlanParameters(){
			...
			if (attrs!=null)
				...
				tmp.put(p.getPre(),p.getPost());
                        ...
                        else {RoundtripLimit=null;WaitingPeriod=null;is529Plan=false;return;}
...
if(is529Plan)
    ToRebalance = compareTargetAndHoldingFunds(PresentativeAssetFundMap);
...
//D:\workspace\LTISystem\WEB-INF\src\com\lti\compiledstrategy\LJF_Strategic_Asset_Allocation_with_RoundTrip_and_5291038.java:511: |πÑ{ã
//~0 java.util.Map<java.lang.String,java.lang.String>
// Å java.util.List<com.lti.type.Pair>
//                                   attrs=_plan.getAttributes();
//                                                            ^
//1 Ô