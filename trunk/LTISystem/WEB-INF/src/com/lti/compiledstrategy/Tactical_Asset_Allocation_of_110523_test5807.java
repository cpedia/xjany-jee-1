...
public class Tactical_Asset_Allocation_of_110523_test5807 extends SimulateStrategy{
	public Tactical_Asset_Allocation_of_110523_test5807(){
		super();
		StrategyID=5807L;
		StrategyClassID=3L;
	}
 ...
 if(AssetRepresentFundMap.containsKey("COMMODITIES") && !_AllowSectorFund && NumberOfMainRiskyClass > 1)  
{
   ...
   if(!hasBasket)
   {
      ...
      for(int i = 0; i < savedCommoFunds.size(); i++)
      {
        ...
        if(getSecurity(f).getAssetClass().getFullName().equalsIgnoreCase("COMMODITIES BROAD BASKET"))
        {
          savedBasketFund = f;
          break;
        }        
...
// 初始化持久化变量
    ...
    boolean needRoundtrip = false;
    boolean needWaitingPeriod = false;
    if(RoundtripLimit.length > 1)
    {
      for(int i = 0; i < RoundtripLimit.length; i++)
        if(RoundtripLimit[i]!=60)
          needRoundtrip = true;
    }
    if(!needRoundtrip)
    {
        RoundtripLimit = new int[1];
        RoundtripLimit[0] = -1;
    }
  
      if(WaitingPeriod.length > 1)
    {
      for(int i = 0; i < WaitingPeriod.length; i++)
        if(WaitingPeriod[i]!=0)
          needWaitingPeriod = true;
    }
    if(!needWaitingPeriod)
    {
        WaitingPeriod = new int[1];
        WaitingPeriod[0] = -1;
    }
