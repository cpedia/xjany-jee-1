...
public class Guru_Asset_Allocation_Indicators634 extends SimulateStrategy{
	public Guru_Asset_Allocation_Indicators634(){
		super();
		StrategyID=634L;
		StrategyClassID=3L;
	}
...
@Override
  public void afterExecuting()   throws Exception {
    ...
    com.lti.util.PersistentUtil.writeGlobalObject(xml,"SmartMoneyIndicator_"+NumberOfGurus +"_" + PortfolioID +".xml",StrategyID,PortfolioID);
  }
@Override
...
String[] PreSelectedModerateFunds = {"AABPX","AABPX","ADBAX","APBAX","BHBFX","BUFBX","CBLFX","DODBX","EXBAX","FASMX","FBALX","FBPBX","FOBAX","FPACX","FPURX","GRSPX","HBFBX","JABAX","LCORX","LKBAX","MAPOX","MNBAX","MOBAX","NCBGX","NOIEX","OAKBX","PRWCX","RIMBX","RPBAX","SSLBX","THPBX","TRPBX","TWBIX","TWSMX","USBSX","USCRX","VALIX","VBINX","VGSTX","VILLX","VSMGX","VWELX","WEBAX","WESRX","BRUFX","BTBFX","CBALX","ELDFX","FAIOX","IBBFX","IDRYX","JANBX","NBGPX","NSDMX"};


if (bAll) {
int i, j;
List<Security> listModerate, listConservative, listWorldAlloc;
int ModerateSize, ConservativeSize, WorldAllocSize;
String[] ModerateFunds;
listModerate=getSecurityByClass(65);
listConservative=getSecurityByClass(66);
listWorldAlloc=getSecurityByClass(99);
ModerateSize =  listModerate.size();
ConservativeSize =  listConservative.size();
WorldAllocSize =  listWorldAlloc.size();

if(NumberOfGurus > 0)
{
    ModerateFunds = PreSelectedModerateFunds; 
     ModerateSize = PreSelectedModerateFunds.length;
}
else
{
    ModerateFunds = new String[ModerateSize];
    for(int k = 0; k < ModerateSize; k++)
    {
        ModerateFunds[k] = listModerate.get(k).getSymbol();
    }
}

class1=new String[ModerateSize + ConservativeSize + WorldAllocSize];
s=class1.length-1;
for (i=0; i<ModerateSize; i++) {
  class1[i]= ModerateFunds[i];
}
j=ModerateSize;
for (i=0;i< ConservativeSize;i++) {
   class1[i+j] = listConservative.get(i).getSymbol();
}
j=j+ConservativeSize;
for (i=0;i< WorldAllocSize;i++) {
   class1[i+j] = listWorldAlloc.get(i).getSymbol();
}
} else {
if(AssetID == 65 && NumberOfGurus > 0)
{
     class1 = PreSelectedModerateFunds; 
}
else
{
...
int MutualFundSize = 0;
boolean[] isMutualFund = new boolean[class1.length];
for(int i = 0; i< class1.length; i++)
{
    try{
    if(getSecurity(class1[i]).getSecurityType() == 4)
    {
        isMutualFund[i] = true;
        MutualFundSize ++;
    }
...
String[] TempFundList = class1;
class1 = new String[MutualFundSize];
int n = 0;
for(int i = 0; i< TempFundList.length; i++)
{
    if(isMutualFund[i])
        {class1[n] = TempFundList[i]; n++;}
}
s=class1.length-1;
...
{
try {
if(getSecurity(class1[i]).getSecurityType() == 4 && LTIDate.before(getEarliestAvaliablePriceDate(getSecurity(class1[i])), LTIDate. getNewNYSEMonth(CurrentDate, -12)))
{
