...
public class Hussman_Peak_PE_Market_Timing_Strategy638 extends SimulateStrategy{
	public Hussman_Peak_PE_Market_Timing_Strategy638(){
		super();
		StrategyID=638L;
		StrategyClassID=9L;
	}
...
@Override
  public void afterExecuting()   throws Exception {
    ...
    com.lti.util.PersistentUtil.writeGlobalObject(xml1,"PeakPE10Data_"+PortfolioID+".xml",StrategyID,PortfolioID);
    com.lti.util.PersistentUtil.writeGlobalObject(xml2,"HussmanRatioList_"+PortfolioID+".xml",StrategyID,PortfolioID);
    com.lti.util.PersistentUtil.writeGlobalObject(currentValue,"HussmanRatioDataValue_"+PortfolioID+".xml",StrategyID,PortfolioID);
    com.lti.util.PersistentUtil.writeGlobalObject(dates[weeks.size()-1],"HussmanRatioDate_"+PortfolioID+".xml",StrategyID,PortfolioID);
    com.lti.util.PersistentUtil.writeGlobalObject(valueState,"HussmanRatioState_"+PortfolioID+".xml",StrategyID,PortfolioID);
  }
@Override
...
double CurrentPE10 = (price*shillerSP500.getCPIRatio())/shillerSP500.getPeakRealEarnings10();
double LongTermPE10 = shillerSP500.getAvgPricePeakEarnings();
...
Ratio = CurrentPE10/LongTermPE10;
 ...
 RatioList.add(Ratio);
 iWeeks++;  
printToLog("iWeeks: "+iWeeks + "   Long Term (peak)PE10 = " + LongTermPE10);
printToLog("iWeeks: "+iWeeks + "   Current (peak)PE10 = " + CurrentPE10);
printToLog("iWeeks: "+iWeeks + "   HussmanPeakPERatio = " + Ratio);

if (SignificantOvervalued < 10.0) {
...
double PE = CurrentPE10;
if (PE < SignificantOvervalued) {
TotalAmount = CurrentPortfolio.getTotalAmount(CurrentDate);
CurrentAsset=new Asset();
CurrentAsset.setAssetStrategyID(getStrategyID("STATIC"));
...
CurrentAsset.setTargetPercentage(0.5);
CurrentPortfolio.addAsset(CurrentAsset);
CurrentPortfolio.buy("Asset", EquityFund, TotalAmount, CurrentDate);
Position=1;
} else {
CurrentAsset=new Asset();
CurrentAsset.setAssetStrategyID(getStrategyID("STATIC"));
...
CurrentAsset.setTargetPercentage(0.5);
CurrentPortfolio.addAsset(CurrentAsset);
CurrentPortfolio.buy("Asset",CashFund, TotalAmount, CurrentDate); 
Position=0;
}
}
...
TotalAmount = CurrentPortfolio.getTotalAmount(CurrentDate);
if (Position==1 && (PE > SignificantOvervalued)) {
   CurrentPortfolio.sellAsset("Asset", CurrentDate);
    CurrentPortfolio.buy("Asset", CashFund, TotalAmount, CurrentDate);
    Position=0;
} else if (Position==0 &&(PE<SignificantUndervalued)) {
   CurrentPortfolio.sellAsset("Asset", CurrentDate);
    CurrentPortfolio.buy("Asset", EquityFund, TotalAmount, CurrentDate);
    Position=1;
}
