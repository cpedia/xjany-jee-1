...
public class Momentum_Scoring_Short636 extends SimulateStrategy{
	public Momentum_Scoring_Short636(){
		super();
		StrategyID=636L;
		StrategyClassID=2L;
	}
	...
	private String[] ShortSecurities;
	public void setShortSecurities(String[] ShortSecurities){
		this.ShortSecurities=ShortSecurities;
	}
	
	public String[] getShortSecurities(){
		return this.ShortSecurities;
	}
	...
	public void fetchParameters()throws ParameterException{
		...
		Securities=(String[])ParameterUtil.fetchParameter("String[]","Securities", "SPY, EFA, EEM, IYR, DJP, TLT, CASH, CASH, CASH", parameters);
		...
		ShortSecurities=(String[])ParameterUtil.fetchParameter("String[]","ShortSecurities", "SDS, EFU, EEV, SRS, CMD, TBT, CASH, CASH, CASH", parameters);
		bShort=(Boolean)ParameterUtil.fetchParameter("boolean","bShort", "true", parameters);
	}
	//----------------------------------------------------
	//variables
	//----------------------------------------------------
...
/* = {"AA", "AXP", "BA", "BAC", "C", "CAT", "CVX", "DD", "DIS", "GE", "GM", "HD", "HPQ", "IBM", "INTC", "JNJ", "JPM", "KFT", "KO", "MCD", "MMM", "MRK", "MSFT", "PFE", "PG", "T", "UTX", "VZ", "WMT", "XOM"};*/
	...
	public void readObject(ObjectInputStream stream) throws IOException, ClassNotFoundException{
		...
		shorts=(String[])stream.readObject();;
	}
...
shorts = new String[Securities.length];

for (int i=0; i<Securities.length; i++) {
class1[i] = Securities[i];
shorts[i]=ShortSecurities[i];
...
for(int i=score.length-1; i>=1; i--)
{
 ...
 Maxscoreindex =i;
   ...
   if(Maxscoreindex !=i)
   {
     ...
     class1 [i]=tmp;
     tmp=shorts[Maxscoreindex];
     shorts[Maxscoreindex]=shorts[i];
     shorts[i]=tmp;
...
if (!shorts[i].equals("CASH")&&bShort) {
    CurrentPortfolio.shortSell("Asset"+i, shorts[i], TotalAmount * (1.0/iTotalHoldings)/2.0, CurrentDate);
} else {
     CurrentPortfolio.buy("Asset"+i, shorts[i], TotalAmount * (1.0/iTotalHoldings), CurrentDate);
}
}
/* CurrentAsset=new Asset();
CurrentAsset.setAssetStrategyID(getStrategyID("STATIC"));
...
CurrentAsset.setAssetStrategyID(getStrategyID("STATIC"));
CurrentAsset.setName("Asset"+i);
//CurrentAsset.setClassID(getAssetClassID(getSecurity (shorts[s-i]).getName()));
CurrentAsset.setTargetPercentage(1.0/iTotalHoldings);
CurrentPortfolio.addAsset(CurrentAsset);
if (!shorts[i].equals("CASH")&&bShort) {
    CurrentPortfolio.shortSell("Asset"+i, shorts[i], TotalAmount * (1.0/iTotalHoldings)/2.0, CurrentDate);
...
//D:\workspace\LTISystem\WEB-INF\src\com\lti\compiledstrategy\Momentum_Scoring_Short636.java:136: ~0&÷
//&÷ ¹Õ getTransactions()
