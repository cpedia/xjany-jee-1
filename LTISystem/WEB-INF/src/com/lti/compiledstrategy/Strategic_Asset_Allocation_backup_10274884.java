...
public class Strategic_Asset_Allocation_backup_10274884 extends SimulateStrategy{
	public Strategic_Asset_Allocation_backup_10274884(){
		super();
		StrategyID=4884L;
		StrategyClassID=3L;
	}
...
@Override
  public void afterExecuting() throws Exception {
    ...
    com.lti.util.PersistentUtil.writeObject(_storeData, "SAA Data Plan " + PlanID);
  }
   ...
   * @return @
   */
  public Object[] DivideAssetClass(Map<String, List<CandidateFundType>> map) throws Exception {
  ...
  public void initStoreData(String dateStr, Map<String, CandidateFundType> CandidateFundMap) throws Exception {
    if(_storeData==null ){
      try {
        _storeData = (Map<String, List<String>>) com.lti.util.PersistentUtil.readObject("SAA Data Plan " + PlanID);
      } catch (Exception e1) {
       }
    }
    ...
    if (_storeData.get(RISKY_GROUP + "." + dateStr) == null || _rewriteData) {
    ...
    /* Iterator<String> iterCan = AvailableFundMap.keySet().iterator();
   while(iterCan.hasNext()){
       String key = iterCan.next();
       if(AvailableFundMap.get(key).isShortHistory())
           printToLog("Short History Fund : " + key);
       else
           printToLog("Normal Available Fund : " + key);
     }*/
  ...
  public void BuildSelectedAsset(Map<String, Double> SelectedAssetPercentagesMap, Map<String, List<String>> AssetRepresentFundMap, Map<String, Double> FundPercentageMap, Map<String, Boolean> CanNotBuyFundMap, String dateStr, Map<String, CandidateFundType> CandidateFundMap)  throws Exception {
    ...
    // Add an asset when the Risky Group or Stable Group is empty
    List<String> RiskyGroupSelect = _storeData.get(RISKY_GROUP + "." + dateStr);
    List<String> StableGroupSelect = _storeData.get(STABLE_GROUP + "." + dateStr);
    ...
    if(_RoundtripLimit != null || _WaitingPeriod != null) {
      ...
      if(roundtripInfo == null && waitingPeriodInfo == null || _rewriteData){
...
// 初始化持久化变量
    ...
    //_is529Plan = true;  // for testing , please delete when deploy
    //_rewriteData =true; // for Test
    ...
    {
       try {
         _holdingInfo =  (Map<String,Map<String,Integer>>) com.lti.util.PersistentUtil.readObject("HoldingInfoPlan_" + PlanID);
       } catch (Exception e1) {}
    }
    ...
    // 初始化CandidateFundMap
    Map<String, CandidateFundType> CandidateFundMap = BuildCandidateFundMap(_WaitingPeriod, _RoundtripLimit, pUseDefaultRedemption, pDefaultRedemptionLimit);
    if (_fundmpt == null) {
...
printToLog("--------------  Re-initial Finish, SAA strategy update to version " + version + " -------------------");
	...
	public void action() throws Exception{
		...
		if ((RebalanceFrequency.equalsIgnoreCase("monthly") && LTIDate.isLastNYSETradingDayOfMonth(LTIDate.getNewDate(CurrentDate, TimeUnit.DAILY, 30 - CheckDate))) || (RebalanceFrequency.equalsIgnoreCase("quarterly") && LTIDate.isLastNYSETradingDayOfQuarter(LTIDate.getNewDate(CurrentDate, TimeUnit.DAILY, 30 - CheckDate))) || (RebalanceFrequency.equals("annually") && LTIDate.isLastNYSETradingDayOfYear(LTIDate.getNewDate(CurrentDate, TimeUnit.DAILY, 30 - CheckDate)))) {
			...
			double aa;
                       ...
                       if(redemptionInfo == null || _rewriteData){
   ...
   // _rewriteData =true; // for Test
			...
			// 初始化CandidateFundMap
			Map<String, CandidateFundType> CandidateFundMap = BuildCandidateFundMap(_WaitingPeriod, _RoundtripLimit, UseDefaultRedemption, DefaultRedemptionLimit);
			if (_fundmpt == null) {
				initmpts(CandidateFundMap);
			}
...
///usr/apache-tomcat-6.0.16/webapps/LTISystem/WEB-INF/src/com/lti/compiledstrategy/Strategic_Asset_Allocation_backup_10274884.java:1185: CanNotBuyFundMap is already defined in ExcludeByRoundTripAndWaiting(java.util.List<java.lang.String>,java.util.Map<java.lang.String,com.lti.type.finance.CandidateFundType>,boolean,java.util.Map<java.lang.String,java.lang.Integer>,java.util.Map<java.lang.String,java.lang.Integer>)
//    Map<String, Boolean> CanNotBuyFundMap = new HashMap<String, Boolean>();
//                         ^
//1 error