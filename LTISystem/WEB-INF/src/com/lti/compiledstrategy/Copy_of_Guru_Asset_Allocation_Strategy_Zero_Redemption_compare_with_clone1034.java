...
public class Copy_of_Guru_Asset_Allocation_Strategy_Zero_Redemption_compare_with_clone1034 extends SimulateStrategy{
	public Copy_of_Guru_Asset_Allocation_Strategy_Zero_Redemption_compare_with_clone1034(){
		super();
		StrategyID=1034L;
		StrategyClassID=3L;
	}
	...
	public void fetchParameters()throws ParameterException{
		...
		RAAInterval=(Integer)ParameterUtil.fetchParameter("int","RAAInterval", "12", parameters);
...
//getVolume function ( used in the Function: Choose_Available_CandidateFunds )
	...
	public void init() throws Exception{
		// RedemptionLimit --> 0; RAAInterval changed to 12; and TimeUnit.MONTH  For 2010/02/26 testing
...
CurrentPortfolio.buy(CurrentAsset.getName(), "CASH", TotalAmount * 0.4,CurrentDate);
	...
	public void action() throws Exception{
		...
		//Since this strategy is used for comparision with Guru AA Clone All; redemption limits are zero, codes about Redemption Limit are deleted
...
for(int i = 0; i < HoldingAssetList.size(); i++)
{
    ...
    for(int j = 0; j < HoldingSecurityList.size(); j++)
    {
        ...
        sa = CurrentPortfolio.getAvailableTradingAmount(sn, 0, CurrentDate,1);// Redemption Limits are modified to 0;
...
double[] WeightDouble = getWeightFromGuru( GuruTargetFunds, AssetBenchmarks, TargetTopN, SharpeMonths,  MoneyAllocation, RAAMethod, RAAInterval, RiskAversion, CurrentDate, TimeUnit.MONTHLY);
...
for (int i=0; i<CurrentAssetList.size();i++){   
    ...
    for(int j=0; j<CurrSecurList.size();j++){   
        if (FavSecurNameList.indexOf(CurrSecurList.get(j).getSymbol())<0)      /*Sell the poor-performance funds as much as possible*/{    
            double SellAmount=CurrentPortfolio.getAvailableTradingAmount(CurrSecurList.get(j).getSymbol(), 0, CurrentDate,1);//Redemption limits are modified to 0;
		...
		double AvailSellAmount;
			...
			if (!CurrSecurList.get(j).getSymbol().equals("CASH")){AvailSellAmount=CurrentPortfolio.getAvailableTradingAmount(CurrSecurList.get(j).getSymbol(), 0, CurrentDate,1);} //Redemption limits are modified to 0;
