...
public class Moving_Average_With_A_Signal488 extends SimulateStrategy{
	public Moving_Average_With_A_Signal488(){
		super();
		StrategyID=488L;
		StrategyClassID=2L;
	}
...
if(LTIDate.isLastNYSETradingDayOfMonth(CurrentDate)) {
   ...
   if(SMA)
     MAValue = S.getSMA(CurrentDate, Interval, TimeUnit.MONTHLY,false);
   else{
     if(preMAValue==0){
       MAValue = S.getEMA(CurrentDate,CurrentDate, Interval, TimeUnit.MONTHLY,false);
       preMAValue=MAValue;
    }
    else{
       preMAValue=MAValue;
       MAValue = (CurrentPrice*2+preMAValue*(Interval-1))/(Interval+1);
		...
		else if (!position&& (CurrentPrice > MAValue) && LTIDate.isLastNYSETradingDayOfMonth(CurrentDate)) {
...
position=true;
		   ...
		   TotalAmount = CurrentPortfolio.getAssetAmount(curAsset, CurrentDate);
CurrentPortfolio.sellAsset(curAsset,CurrentDate);
CurrentPortfolio.buy (curAsset, CashSecurity, TotalAmount, CurrentDate);
position=false;
		}
