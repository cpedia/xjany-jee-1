...
public class Market_Timing_Rule_with_Short_Term_Interest_Rate556 extends SimulateStrategy{
	public Market_Timing_Rule_with_Short_Term_Interest_Rate556(){
		super();
		StrategyID=556L;
		StrategyClassID=9L;
	}
...
if(Threshold>1)
   ThresholdValue=Bill.getSMA(CurrentDate,(int)Threshold,TimeUnit.DAILY)/100;


if(Bill.getCurrentPrice(CurrentDate)/100<ThresholdValue)
...
if(DelayDay>=2)
{
    for(int i=2;i<=DelayDay;i++)
    {
        ...
        if(Threshold>1)
        {
            if(Bill.getCurrentPrice(PastDate)/100<Bill.getSMA(PastDate,(int)Threshold,TimeUnit.DAILY))
        ...
        {
            if(Bill.getCurrentPrice(PastDate)/100<ThresholdValue)
                SwitchSignal[DelayDay-i]=1;
            else
                SwitchSignal[DelayDay-i]=0;
        } 
