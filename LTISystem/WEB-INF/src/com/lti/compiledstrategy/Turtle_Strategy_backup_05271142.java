...
public class Turtle_Strategy_backup_05271142 extends SimulateStrategy{
	public Turtle_Strategy_backup_05271142(){
		super();
		StrategyID=1142L;
		StrategyClassID=14L;
	}
...
public void calculateVolatility() throws Exception {
	...
	for(int j=0;j<n;j++){
		LastClose[j]=Fund[j].getCurrentPrice(LastDate);
		CurrentHigh[j]=Fund[j].getHigh(CurrentDate);
		CurrentLow[j]=Fund[j].getLow(CurrentDate);
...
for(int i=0;i<n;i++){
	Fund[i]=getSecurity(nameArray[i]);
	CurrentPrice[i]=Fund[i].getCurrentPrice(CurrentDate);
	Established[i]=false;
	LastEarned[i]=false;
	countUnit[i]=0;
	incUnit[i]=0;
...
for(int j=0;j<n;j++){                          //Caculating the initial Volotility of 20 days moving average TR.
	...
	for(int k=0;k<20;k++){
		LastClose[j]=Fund[j].getCurrentPrice(StartDate);
		CurrentHigh[j]=Fund[j].getHigh(StartDate);
		CurrentLow[j]=Fund[j].getLow(StartDate);
...
CurrentPrice[i]=Fund[i].getCurrentPrice(CurrentDate);
for(int k=0;k<n;k++){
	Last10DaysHigh[k]=Fund[k].getHighestPrice(Start10Days,LastDate);
	Last10DaysLow[k]=Fund[k].getLowestPrice(Start10Days,LastDate);
	Last20DaysHigh[k]=Fund[k].getHighestPrice(Start20Days,LastDate);
	Last20DaysLow[k]=Fund[k].getLowestPrice(Start20Days,LastDate);
	Last55DaysHigh[k]=Fund[k].getHighestPrice(Start55Days,LastDate);
	Last55DaysLow[k]=Fund[k].getLowestPrice(Start55Days,LastDate);
}
calculateVolatility();
calculateUnit();
