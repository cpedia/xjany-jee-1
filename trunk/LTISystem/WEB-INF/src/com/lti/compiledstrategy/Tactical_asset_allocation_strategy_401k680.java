...
public class Tactical_asset_allocation_strategy_401k680 extends SimulateStrategy{
	public Tactical_asset_allocation_strategy_401k680(){
		super();
		StrategyID=680L;
		StrategyClassID=3L;
	}
...
/*delete the duplicate candidate fund and those we can not find their asset class or benchmark*/

int[] Delete = new int[CandidateFund.length];
int k = 0;
...
if(s1>0)
{
    ...
    for(int i=0; i<s1;i++)
    {
        ...
        printToLog("sell " + SellList1[i] +  "  Amount  " + SecurityAmount);
...
if(s2>0)
{
    ...
    for(int i=0; i<s2;i++)
    {
        ...
        printToLog("sell " + SellList2[i] +  "  Amount  " + SecurityAmount);
        }
...
printToLog( "Sell 1 = " + s1 + "Sell 2= " +  s2 + " Cash Amount  = " + CurrentPortfolio.getCash());
if(s1>0 || s2>0)
{
