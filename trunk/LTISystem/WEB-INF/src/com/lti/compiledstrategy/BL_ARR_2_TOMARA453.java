...
public class BL_ARR_2_TOMARA453 extends SimulateStrategy{
	public BL_ARR_2_TOMARA453(){
		super();
		StrategyID=453L;
		StrategyClassID=3L;
	}
	...
	public void fetchParameters()throws ParameterException{
		...
		tu=(TimeUnit)ParameterUtil.fetchParameter("TimeUnit","tu", "TimeUnit.YEARLY", parameters);
		v=(Integer)ParameterUtil.fetchParameter("int","v", "3", parameters);
		Period=(Integer)ParameterUtil.fetchParameter("int","Period", "1", parameters);
	}
	//----------------------------------------------------
	//variables
	//----------------------------------------------------
...
TopSecurityArray=getTopSecurity(list2,v,-1*Period,CurrentDate,tu,SortType.SHARPE,true);
double[] Q=new double[v];
for(int i=0;i<v;i++)
{
Q[i]=TopSecurityArray.get(i). getReturn ( CurrentDate,tu,-1,false);
}
...
temp=RAA(T,CurrentDate,TimeUnit.DAILY,TopSecurityArray.get(i).getName(),Index);
for(int j=0;j<K;j++)
{
P[i][j]=temp[j];
}
