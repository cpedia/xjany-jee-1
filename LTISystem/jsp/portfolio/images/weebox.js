	...
	var weebox = function(content, options) {
		...
		this.initBox = function() {
			...
			if (self.options.type == 'wee') {
				html = 	'<div class="weedialog">' +
						...
						'		<div class="dialog-tr">' +
						'			<div class="dialog-close" title="鍏抽棴"><a href="#"></a></div>' +
						'		</div>' +
						'	</div>' +
						...
						'				<input type="button" class="dialog-ok" value="纭" />&nbsp;' +
						'				<input type="button" class="dialog-cancel" value="鍙栨秷" />' +
						'			</div>' +
						'		</div>' +
						'	</div>' +	
				...
				html = "<div class='dialog-box'>" +
							...
							"<div class='dialog-button'>" +
								"<input type='button' class='dialog-ok' value='纭畾'>" +
								"<input type='button' class='dialog-cancel' value='鍙栨秷'>" +
							"</div>" +
						"</div>";
			}
		...
		this.initContent = function(content) {
			...
			if (self.options.contentType == "selector") {
				...
				//if have input do
				var is = $(self.selector).find(':input');
				self.dh.find('.dialog-content').find(':input').each(function(i){
					this.value = is[i].value;
				});			
				$(self.selector).empty();
				self.onopen();
				self.show();
		...
		this.initEvent = function() {
			...
			if (typeof(self.options.onok) == "function") {
				self.dh.find(".dialog-ok").unbind('click').click(function(){
		...
		this.close = function() {
			...
			if (self.options.contentType == 'selector') {
				if (self.options.contentChange) {
					...
					//if have input do
					var is = self.find(':input');
					//$(self.selector).html(self.getContent());						
					if (is.length > 0) {
						$(self.selector).find(':input').each(function(i){
							this.value = is[i].value;
						});
					}
				} else {
