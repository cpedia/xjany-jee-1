...
public class Tactical_Asset_Allocation1003 extends SimulateStrategy{
	public Tactical_Asset_Allocation1003(){
		super();
		StrategyID=1003L;
		StrategyClassID=3L;
	}
...
public void initmpts(Map<String, CandidateFundType> CandidateFundMap)  throws Exception {
  ...
  public Map<String, CandidateFundType> ChooseAvailableCandidate(Map<String, CandidateFundType> CandidateFundMap)   throws Exception {
    ...
    while (iter.hasNext()) {
      ...
      if (Fund == null || symbol.equals("SSSFX")) {
        printToLog("Can not get the security, so exclude = " + symbol);
        continue;
      }

      try {
        if (Fund.getIsClosed() && LTIDate.equals(Fund.getEndDate(), CurrentDate))
        {printToLog("The security is closed, so exclude = " + symbol);   continue;}
      } catch (Exception e3) {
        printToLog(symbol + "'s EndDate is probably null, so it is excluded.");
        continue;
      }
      ...
      if (Bench == null)
      {printToLog("fail to get the security's benchmark, so exclude = " + symbol);  continue;}
      // Omit those funds that DO NOT have currentprice
      try {
        Fund.getAdjClose(CurrentDate);
      } catch (Exception e) {
        printToLog("fail to get the security's current price, so exclude = " + symbol);
        continue;
      }
      ...
      try {
        ...
        printToLog("fail to get the price of the security's benchmark, the security = " + symbol);
        BenchHasPrice = false;
      }
      ...
      if (LTIDate.before(Fund.getStartDate(), LTIDate.getNewNYSEMonth(CurrentDate, -12))) {
       // candidateFund.setInAfterDateFilter(true);
        ...
        if (!_IncludeAllETF && LTIDate.before(Bench.getStartDate(), LTIDate.getNewNYSEMonth(CurrentDate, -12))) {
          ...
          printToLog(symbol + " is included no matter of SD");
        ...
        }else if(Fund.getSecurityType() == 1)
          printToLog(symbol + " is included no matter of Volumn");
...
/*       Iterator<String> iteratorT1 =AvailableFundMap.keySet().iterator();
      while(iteratorT1.hasNext()){
           String f = iteratorT1. next();
        printToLog("Available funds: " + f);
      }
   */
      
      Map<String, List<CandidateFundType>> AllAssetFundsMap = getAvailableAssetClassSet(MainAssetClass, AvailableFundMap, true);

/*      printToLog("MainAssetClass : " + MainAssetClass);
      Iterator<String> iteratorT2 =AllAssetFundsMap.keySet().iterator();
      while(iteratorT2.hasNext()){
           String asset = iteratorT2.next();
           List<CandidateFundType> fundList = AllAssetFundsMap.get(asset);
        printToLog("Available asset: " + asset);
        for(int i = 0; i < fundList.size(); i++)
          printToLog("include fund: " + fundList.get(i).getSymbol());
      }
*/      
  ...
  //adjust the allocation to the volatile commodity funds, added by WYJ on 04/29/2011
 if(AssetRepresentFundMap.containsKey("COMMODITIES") && !_AllowSectorFund && NumberOfMainRiskyClass > 1)  
{
   double movePercentage = 0;
   boolean hasBasket = false;
   // accumulate the percentage needed to move
   List<String>  cFundList = AssetRepresentFundMap.get("COMMODITIES");
   List<String>  basketFunds = new ArrayList<String>();
   for(int i = 0; i < cFundList.size(); i++)
   {
     String fund = cFundList.get(i);
      if(getSecurity(fund).getAssetClass().getName().equalsIgnoreCase("COMMODITIES BROAD BASKET"))
      {
        hasBasket = true;
        basketFunds.add(fund);
      }
      else{
         double tempPerc = FundPercentageMap.get(fund);
         if(tempPerc/2 > MinimumBuyingPercentage)
         {
           FundPercentageMap.put(fund, tempPerc/2);
           movePercentage += tempPerc/2;
         }else{
           FundPercentageMap.put(fund, MinimumBuyingPercentage);
           movePercentage += (tempPerc - MinimumBuyingPercentage);
         }
      }
   }
   // find the commodities basket fund in the dataobject if no such fund is in the selected fund list
   String savedBasketFund = null;
   if(!hasBasket)
   {
      List<String> savedCommoFunds = _storeData.get("COMMODITIES"+"."+dateStr);
      for(int i = 0; i < savedCommoFunds.size(); i++)
      {
        String f = savedCommoFunds.get(i);
        //printToLog("test 0527: Fund: " + f);
        //printToLog("test 0527: Fund: " + f);
        if(getSecurity(f).getAssetClass().getName().equalsIgnoreCase("COMMODITIES BROAD BASKET"))
        {
          savedBasketFund = f;
          break;
        }        
   ...
   // move the percentage to Commodities Basket Funds or the other risky assets
   if(hasBasket)        // if has DBC in the selected funds
   {
      double weight2 = movePercentage/basketFunds.size();
      for(int i = 0; i < basketFunds.size(); i++)
      {
          String f = basketFunds.get(i);
          double tempPerc = FundPercentageMap.get(f);
          FundPercentageMap.put(f, tempPerc + weight2);                
      }  
   }
   else if(savedBasketFund != null && movePercentage > MinimumBuyingPercentage){                // if has basket fund in the _storeData, need improvement (DBC not in the can not buy fund list)
       cFundList.add(savedBasketFund);
       FundPercentageMap.put(savedBasketFund, movePercentage);
   }else{
       double weight1 = movePercentage/(tmpRiskyGroupSelect.size()-1);    // equally allocate to the other risky assets
       Iterator<String> iter5 =  AssetRepresentFundMap.keySet().iterator();
       while(iter5.hasNext()){
          String asset = iter5.next();
          if(!asset.equalsIgnoreCase("COMMODITIES") && tmpRiskyGroupSelect.indexOf(asset) >-1) // the selected risky assets except commodities
          {
            List<String> fundList = AssetRepresentFundMap.get(asset);
            double weight2 = weight1/fundList.size();   // equally allocate to the funds in the risky asset
            for(int i = 0; i < fundList.size(); i++)
            {
               String fund = fundList.get(i);
               double tempPerc = FundPercentageMap.get(fund);
               FundPercentageMap.put(fund, tempPerc + weight2);
            }
            double assetTempPerc = SelectedAssetPercentagesMap.get(asset);
            SelectedAssetPercentagesMap.put(asset, assetTempPerc + weight1);
          }     
       } // end of "iter5"
       double assetTempPerc = SelectedAssetPercentagesMap.get("COMMODITIES");
       SelectedAssetPercentagesMap.put("COMMODITIES", assetTempPerc - movePercentage);       
   }  //end of "hasBasket else"
   printToLog("adjust the commodity percentage: " + FundPercentageMap);
 } // end of adjust for commodities

    printToLog("SelectedAssetPercentagesMap after Removing zeros :" + SelectedAssetPercentagesMap);
  }  // End of BuildSelectedAsset
  ...
  public Map<String, Boolean> ExcludeByRoundTripAndWaiting(List<String> NeedTestFundList, Map<String, CandidateFundType> CandidateFundMap, boolean NoInfo, Map<String,Integer> roundtripInfo, Map<String,Integer> waitingPeriodInfo)  throws Exception {
    ...
    for (int i = 0; i < NeedTestFundList.size(); i++) {
      ...
      printToLog("Fund deal in ExcludeByRoundTripAndWaiting = " + fund);
...
//Adj for Roungtrip Part 0 LJF and adj for 529(actually Initial for getParameter Part)
    ...
    private void initPlanParameters(){
      ...
      if(tmp.get("IncludeAllETF")!=null && tmp.get("IncludeAllETF").trim().equalsIgnoreCase("true")){
        _IncludeAllETF=true;
        printToLog("This portfolio will not exclude ETFs with small volumn or large volatility.");
        }
...
// 初始化持久化变量
    ...
    // 初始化CandidateFundMap
    Map<String, CandidateFundType> CandidateFundMap = BuildCandidateFundMap(WaitingPeriod, RoundtripLimit);
    if(_fundmpt==null){
...
long t1=System.currentTimeMillis();
		...
		//try{
if((CheckFrequency.equalsIgnoreCase("daily") && LTIDate.isNYSETradingDay(CurrentDate)) || ( CheckFrequency.equalsIgnoreCase("monthly") && LTIDate.isLastNYSETradingDayOfMonth(LTIDate.getNewDate(CurrentDate, TimeUnit.DAILY, 31 - CheckDate))) 
|| ( CheckFrequency.equalsIgnoreCase("quarterly") && LTIDate.isLastNYSETradingDayOfQuarter(LTIDate.getNewDate(CurrentDate, TimeUnit.DAILY, 31 - CheckDate))) || (CheckFrequency.equalsIgnoreCase("annually")&&LTIDate.isLastNYSETradingDayOfYear(LTIDate.getNewDate(CurrentDate, TimeUnit.DAILY, 31 - CheckDate))) ||  (CheckFrequency.equalsIgnoreCase("weekly")&&LTIDate.isLastNYSETradingDayOfWeek(CurrentDate)) ) {
			long sTime3 = System.currentTimeMillis();

                        sendMail();     // To add a default transaction record for sending email alert;  added on 2011-03-14
                        String dateStr = _df.format(CurrentDate);
			...
			double aa;
                       ...
                       if(_holdingInfo  == null || !UseDataObject)
                          _holdingInfo = new HashMap<String,Map<String,Integer>>();
...
printToLog("Test0501: TotalAvailableCapital = " + TotalAvailableCapital + "  TotalSellPercentageOfStableAsset = " + TotalSellPercentageOfStableAsset);
 			...
 			for (int i = 0; i < HoldingAssetList.size(); i++) {
                                ...
                                if(assetName.equals("CASH") || assetName.equals("INTERNATIONAL BONDS") || assetName.equals("FIXED INCOME"))
				    totalCurrentStableAssetPerc += CurrentPortfolio.getAssetAmount(assetName, CurrentDate) / ta;    
                        } 
...
printToLog("test0501: totalCurrentStableAssetPerc = " + totalCurrentStableAssetPerc + "StablePercentageShouldHold = " + StablePercentageShouldHold+ "  LeastStableToBuy = "  + LeastStableToBuy );
  ...
  printToLog("test0501 : Asset = " + AssetName  +  "  to buy = " + toBuy);
 ...
 printToLog("test0501 : Asset Actual Trading Percentage (after stable allocation) : " + AssetActualTradingPercentage);
                      

                     //  if we have available capital, then decide how much to buy Risky Asset
                    boolean toBuyRiskyAsset = true;
                    if(TotalAvailableCapital <= 0)
                        toBuyRiskyAsset = false;
				...
				while (iter7.hasNext()) {
					AssetName = iter7.next();
					    ...
					    if (ActualPercentage <= TargetPercentage)
                                ...
                                // how much we can buy risky asset
			double BuyWeight = TotalAvailableCapital / TotalTargetBuyOfRiskyAsset;
printToLog("test0501 : (after stable allocation) TotalAvailableCapital = " + TotalAvailableCapital +  " TotalTargetBuyOfRiskyAsset = " + TotalTargetBuyOfRiskyAsset);
