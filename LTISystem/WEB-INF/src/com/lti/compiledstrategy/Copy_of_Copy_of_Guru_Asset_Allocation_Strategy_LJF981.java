...
public class Copy_of_Copy_of_Guru_Asset_Allocation_Strategy_LJF981 extends SimulateStrategy{
	public Copy_of_Copy_of_Guru_Asset_Allocation_Strategy_LJF981(){
		super();
		StrategyID=981L;
		StrategyClassID=3L;
	}
...
/*Calculate weights with function_getWeightFromGuru*/
...
for (int i=0; i<CurrentAssetList.size();i++){   
    ...
    for(int j=0; j<CurrSecurList.size();j++){   
        if (FavSecurNameList.indexOf(CurrSecurList.get(j).getSymbol())<0)      /*Sell the poor-performance funds as much as possible*/{    
            double SellAmount=CurrentPortfolio.getAvailableTradingAmount(CurrSecurList.get(j).getSymbol(), RedemptionMonthMap.get(CurrSecurList.get(j).getSymbol()), CurrentDate,false);
		...
		double AvailSellAmount;
			...
			if (!CurrSecurList.get(j).getSymbol().equals("CASH")){AvailSellAmount=CurrentPortfolio.getAvailableTradingAmount(CurrSecurList.get(j).getSymbol(), RedemptionMonthMap.get(CurrSecurList.get(j).getSymbol()), CurrentDate,false);}
