...
public class Strategic_Asset_Allocation_backup_1004211093 extends SimulateStrategy{
	public Strategic_Asset_Allocation_backup_1004211093(){
		super();
		StrategyID=1093L;
		StrategyClassID=3L;
	}
...
//TargetPercentage is 85, or 15, NOT 0.85,0.15
...
/* Choose_Available_CandidateFunds (Version 2010.04.05)*/
...
for(int i=0; i< CandidateFund.length; i++)
{
	...
	if(LTIDate.before(getEarliestAvaliablePriceDate(getSecurity(CandidateFund[i])), LTIDate.getNewNYSEMonth(CurrentDate, -SharpeMonths)) && LTIDate.before(getEarliestAvaliablePriceDate(Bench), LTIDate. getNewNYSEMonth(CurrentDate, -SharpeMonths)))
	{
		...
		if ( PutThisFund ) {CandidateFundList.add(CandidateFund[i]);/*testing*/printToLog(Fund+"---"+Fund.getAssetClass().getName());}
...
CurrentAsset.setTargetPercentage(WeightsInPercent.get(i).doubleValue());
...
double TotalAmount=CurrentPortfolio.getTotalAmount(CurrentDate);
printToLog("CurrentPortfolio.getTotalAmount= "+CurrentPortfolio.getTotalAmount());
...
while (iter.hasNext())
{
	...
	if (AssetAmountMap.get(CurrentAsset.getName())!= null){Decimal=CurrentAsset.getTargetPercentage()/100-AssetAmountMap.get(CurrentAsset.getName()).doubleValue()/TotalAmount;}
