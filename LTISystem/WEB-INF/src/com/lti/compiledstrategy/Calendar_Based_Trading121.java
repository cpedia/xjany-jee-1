...
public class Calendar_Based_Trading121 extends SimulateStrategy{
	public Calendar_Based_Trading121(){
		super();
		StrategyID=121L;
		StrategyClassID=9L;
	}
...
CurrentPortfolio.buy (curAsset, "CASH", amount, CurrentDate);
	...
	public void action() throws Exception{
		...
		/*printToLog("date is "+CurrentDate);
printToLog("bRule1 bRule2 bRule3 " + bRule1+bRule2+bRule3);
printToLog("LTIDate.getNewNYSETradingingDay(CurrentDate, 2) "+ LTIDate.getNewNYSETradingDay(CurrentDate,2));*/
...
else if (LTIDate.isNYSEHoliday(LTIDate.getNewWeekDay(CurrentDate,1))){
	InterSellRule2=true;
	if(InterSellRule1){
		if(!LTIDate.isWeekDay(CurrentDate, Calendar.FRIDAY)&&!LTIDate.isWeekDay(CurrentDate, Calendar.WEDNESDAY)&&!LTIDate.isThirdFridayofMonth(LTIDate. getNewWeekDay (CurrentDate,-1))){
		double amount=CurrentPortfolio.getAssetAmount(curAsset, CurrentDate);
if(ruleNameRecord==1){
...
else if(LTIDate.isFirstNYSETradingDayofMonth (LTIDate.getNewNYSETradingDay(CurrentDate, -4))){
	InterSellRule1=true;
	if(InterSellRule2){
		double amount=CurrentPortfolio.getAssetAmount(curAsset, CurrentDate);
if(ruleNameRecord==1){
