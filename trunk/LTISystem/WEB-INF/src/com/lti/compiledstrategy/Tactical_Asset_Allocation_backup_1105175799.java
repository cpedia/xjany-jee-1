...
public class Tactical_Asset_Allocation_backup_1105175799 extends SimulateStrategy{
	public Tactical_Asset_Allocation_backup_1105175799(){
		super();
		StrategyID=5799L;
		StrategyClassID=3L;
	}
 ...
 if(AssetRepresentFundMap.containsKey("COMMODITIES") && !_AllowSectorFund && NumberOfMainRiskyClass > 1)  
{
   ...
   // move the percentage to DBC or the other risky asset
   if(hasDBC)        // if has DBC in the selected funds
   {
      double tempPercDBC = FundPercentageMap.get("DBC");
      FundPercentageMap.put("DBC", tempPercDBC + movePercentage);                  
   }
   else if(CandidateFundMap.containsKey("DBC") && movePercentage > MinimumBuyingPercentage){                // if has DBC in the candidate funds, need improvement (DBC not in the can not buy fund list)
       cFundList.add("DBC");
       FundPercentageMap.put("DBC", movePercentage);
   }else{
