...
public class Copy_of_TAA_roundtrip_consistance_test1378 extends SimulateStrategy{
	public Copy_of_TAA_roundtrip_consistance_test1378(){
		super();
		StrategyID=1378L;
		StrategyClassID=3L;
	}
...
//Adj for Roungtrip Part 0 LJF Initial 
		...
		private void initPlanParameters(){
			...
			String[] tmpRoundtrip=tmp.get("RoundtripLimit").split(",");
			if (tmpRoundtrip.length==CandidateFund.length){
			...
			String[] tmpWaiting=tmp.get("WaitingPeriod").split(",");
			if(tmpWaiting.length==CandidateFund.length){
...
AssetRepresentFundMap.put(SelectedAssets.get(i).trim(), RepresentFundList);
	...
	for(int iasset=0;iasset<NeedTestAssetList.size();iasset++)
	{
		...
		for (int ifund=0;ifund<RepresentFundList.size();ifund++)
		{
			...
			{
				if (CurrentPortfolio.getSecurityAmount(AssetName,FundName,CurrentDate)/CurrentPortfolio.getTotalAmount() < FundPercentageMap.get(FundName)) // Detect for CanNotBuy Funds that may need for Buy Rebalance Action
				{
					double TempPercentage=FundPercentageMap.get(FundName);
					double ActualPercentage=CurrentPortfolio.getSecurityAmount(AssetName,FundName,CurrentDate)/CurrentPortfolio.getTotalAmount();
				...
				else {printToLog("LJF Important Notice: CurrentHolding has this CanNotBuyFund "+FundName+", but its actual percentage"+(CurrentPortfolio.getSecurityAmount(AssetName,FundName,CurrentDate)/CurrentPortfolio.getTotalAmount())+" > Target percentage "+FundPercentageMap.get(FundName));}
			}
		}
...
///usr/apache-tomcat-6.0.16/webapps/LTISystem/WEB-INF/src/com/lti/compiledstrategy/Copy_of_TAA_roundtrip_consistance_test1378.java:582: incompatible types
