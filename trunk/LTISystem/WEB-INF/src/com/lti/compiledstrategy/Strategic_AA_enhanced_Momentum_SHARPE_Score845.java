...
public class Strategic_AA_enhanced_Momentum_SHARPE_Score845 extends SimulateStrategy{
	public Strategic_AA_enhanced_Momentum_SHARPE_Score845(){
		super();
		StrategyID=845L;
		StrategyClassID=3L;
	}
	...
	public void fetchParameters()throws ParameterException{
		...
		SortMethod=(String)ParameterUtil.fetchParameter("String","SortMethod", "SHARPE", parameters);
	}
	//----------------------------------------------------
	//variables
	//----------------------------------------------------
...
List<String> AvailableAssetClassList = new ArrayList<String>();
	...
	//----------------------------------------------------	
	//Functions
public HashMap<String,List<String>> getPresentativeAssetFunds(HashMap<String,List<String>> AllAssetFundsMap,  List<String> AvailableAssetClassList, int MaxOfEachAsset)
 throws Exception {
...
for(int i =0; i< AvailableAssetClassList.size(); i++)
{
  ...
  else
  {
	...
	//Using Momentum Score to select funds
	double[] FundsScore=getMomemtumScore(Funds,CurrentDate);
	int FundNumber = MaxOfEachAsset > Funds.length ? Funds.length : MaxOfEachAsset ;
	for (int i1=0;i1<FundNumber;i1++){
		int Index=0;
		for (int i2=0;i2<Funds.length;i2++){
			if (PresentativeFundList.indexOf(Funds[i2])==(-1) && FundsScore[Index]<FundsScore[i2]) {Index=i2;}
		}
		PresentativeFundList.add(Funds[Index]);
	}
    PresentativeAssetFundMap.put(AvailableAssetClassList.get(i), PresentativeFundList);
  }
}
...
public double[] getMomemtumScore(String[] Securities, Date CurrentDate) throws Exception {
double[] score=new double[Securities.length];
for(int i=0; i<score.length; i++)
	{ score[i]=0;}

double a;
double bp1=0; double bp2=0; double bp3=0; double bp4=0;

String[] newArray1=getTopSecurityArray(Securities,-12, CurrentDate, TimeUnit.MONTHLY, SortType.SHARPE, true);
String[] newArray2=getTopSecurityArray(Securities, -6, CurrentDate, TimeUnit.MONTHLY, SortType.SHARPE, true);
String[] newArray3=getTopSecurityArray(Securities, -3, CurrentDate, TimeUnit.MONTHLY, SortType.SHARPE, true);
String[] newArray4=getTopSecurityArray(Securities, -1, CurrentDate, TimeUnit.MONTHLY, SortType.SHARPE, true);
...
{
a =(getSecurity(Securities[i]).getSharpeRatio( -12, CurrentDate, TimeUnit.MONTHLY, true)
+ getSecurity(Securities[i]).getSharpeRatio(-6,CurrentDate, TimeUnit.MONTHLY, true)
+ getSecurity(Securities[i]).getSharpeRatio(-3,CurrentDate, TimeUnit.MONTHLY, true)
+ getSecurity(Securities[i]).getSharpeRatio(-1,CurrentDate, TimeUnit.MONTHLY, true) )*100/4;
...
HashMap<String,List<String>>  PresentativeAssetFundMap = new HashMap<String,List<String>>();
PresentativeAssetFundMap = getPresentativeAssetFunds(AllAssetFundsMap,  AvailableAssetClassList, MaxOfEachAsset);
