...
public class Tactical_Asset_Allocation_scailing_reinitial1150 extends SimulateStrategy{
	public Tactical_Asset_Allocation_scailing_reinitial1150(){
		super();
		StrategyID=1150L;
		StrategyClassID=3L;
	}
     ...
     if(getEarliestAvaliablePriceDate(getSecurity(CandidateFund[i])) != null)
      ...
      // Omit those Funds that 12-month S.D> 1.5*Benchmark  (Version 2010.02.04)
			...
			if (FundSD > BenchSD * 1.5) 
				{PutThisFund = false; TooVolatile[i] = 1;printToLog("Too Volatile ; exclude "+ CandidateFund[i]); continue;} //TYPE 2
...
if(MaxOfStableAsset  > 1 )
{
    ...
    {    
         ...
         OriginalFundList =  AllAssetFundsMap.get(StableGroupSelect.get(i));
...
for(int i = 0; i < SelectedAssets.size(); i++)
{
    ...
    for(int j = 0; j< FundList.size(); j++)
    {
        ...
        else
        {
            ...
            PseudoMap.put(Funds[j], FundList.get(j));
...
for(int j = 0; j < RepresentNumber; j++)  
{
    if(!Funds[j].equals("CASH"))
    {
        ...
        {
            TempWeight = FundPercentageMap.get(Funds[j]);
            TempWeight += Weight;
            FundPercentageMap.put(Funds[j], TempWeight);
...
for(int i = 0; i < TempAssetList.size(); i++)
{
    if(TempAssetPercentages.get(i) > 0 &&  AssetRepresentFundMap.get(TempAssetList.get(i)) != null)
    {
        SelectedAssets.add(TempAssetList.get(i));
        SelectedAssetPercentages.add(TempAssetPercentages.get(i));
    }
...
for(int i = 0; i < SelectedAssets.size(); i++)
{
    ...
    if((AvailableTradingPercentage <= ActualPercentage - TargetPercentage  && AvailableTradingPercentage != 0) || PresentativeSecurityList == null || PresentativeSecurityList.size() == 0)
    {
...
while(iter1.hasNext() && !Done)
{
    ...
    HashMap<String, Double> TempTradingPercentageMap = AssetSecurityTradingPercentageMap.get(Asset);
    iter2 = TempTradingPercentageMap.keySet().iterator();
    while(iter2.hasNext() && !Done)
    {
        String Security = (String)iter2.next();
        TempPercentage = TempTradingPercentageMap.get(Security);
        if(TempPercentage > 0)
        {
            TempPercentage += UnTradePercentage;
            TempTradingPercentageMap.put(Security, TempPercentage);
            AssetSecurityTradingPercentageMap.put(Asset, TempTradingPercentageMap);
            Done = true;
        }
    }
}
...
///usr/apache-tomcat-6.0.16/webapps/LTISystem/WEB-INF/src/com/lti/compiledstrategy/Tactical_Asset_Allocation_scailing_reinitial1150.java:1047: cannot find symbol
//symbol  : variable StableAllocation
//location: class com.lti.compiledstrategy.Tactical_Asset_Allocation_scailing_reinitial1150
//StableAllocation = RiskProfile;
//^
///usr/apache-tomcat-6.0.16/webapps/LTISystem/WEB-INF/src/com/lti/compiledstrategy/Tactical_Asset_Allocation_scailing_reinitial1150.java:1048: cannot find symbol
//symbol  : variable RiskyAllocation
//location: class com.lti.compiledstrategy.Tactical_Asset_Allocation_scailing_reinitial1150
//RiskyAllocation = 100 - RiskProfile;
//^
///usr/apache-tomcat-6.0.16/webapps/LTISystem/WEB-INF/src/com/lti/compiledstrategy/Tactical_Asset_Allocation_scailing_reinitial1150.java:1288: cannot find symbol
//symbol  : variable RiskyAllocation
//location: class com.lti.compiledstrategy.Tactical_Asset_Allocation_scailing_reinitial1150
//    SelectedAssetPercentages.add(RiskyAllocation/RiskyGroupSelect.size()/100);
//                                 ^
...
//    SelectedAssetPercentages.add(RiskyAllocation/RiskyGroupSelect.size()/100);
//                            ^
///usr/apache-tomcat-6.0.16/webapps/LTISystem/WEB-INF/src/com/lti/compiledstrategy/Tactical_Asset_Allocation_scailing_reinitial1150.java:1295: cannot find symbol
//symbol  : variable RiskyAllocation
//location: class com.lti.compiledstrategy.Tactical_Asset_Allocation_scailing_reinitial1150
//             SelectedAssetPercentages.set(j, (RiskyAllocation/RiskyGroupSelect.size() + StableAllocation/StableGroupSelect.size())/100 );
//                                              ^
...
//             SelectedAssetPercentages.set(j, (RiskyAllocation/RiskyGroupSelect.size() + StableAllocation/StableGroupSelect.size())/100 );
//                                                                                        ^
///usr/apache-tomcat-6.0.16/webapps/LTISystem/WEB-INF/src/com/lti/compiledstrategy/Tactical_Asset_Allocation_scailing_reinitial1150.java:1295: set(int,java.lang.Double) in java.util.List<java.lang.Double> cannot be applied to (int,int)
//             SelectedAssetPercentages.set(j, (RiskyAllocation/RiskyGroupSelect.size() + StableAllocation/StableGroupSelect.size())/100 );
//                                     ^
///usr/apache-tomcat-6.0.16/webapps/LTISystem/WEB-INF/src/com/lti/compiledstrategy/Tactical_Asset_Allocation_scailing_reinitial1150.java:1302: cannot find symbol
//symbol  : variable StableAllocation
//location: class com.lti.compiledstrategy.Tactical_Asset_Allocation_scailing_reinitial1150
//        SelectedAssetPercentages.add(StableAllocation/StableGroupSelect.size()/100);
//                                     ^
