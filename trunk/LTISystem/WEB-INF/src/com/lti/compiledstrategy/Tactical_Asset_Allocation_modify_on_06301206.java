...
public class Tactical_Asset_Allocation_modify_on_06301206 extends SimulateStrategy{
	public Tactical_Asset_Allocation_modify_on_06301206(){
		super();
		StrategyID=1206L;
		StrategyClassID=3L;
	}
...
//Add Long-Term Bond to the MainAssetClass 
MainAssetClassList.add("Long-Term Bond");

HashMap<String,List<String>> AllAssetFundsMap = getAvailableAssetClassSet(MainAssetClassList, CandidateFundList, true); // true means to treat Long-Term Bond as a Main Asset Class, 0630  
...
printToLog("------------Short History Fund List  ---------------");
for(int i = 0; i < ShortHistoryFundList.size(); i++)
    printToLog(ShortHistoryFundList.get(i));
...
double MaxScore;
int MaxScoreIndex;
for(int l=0; l<=Funds.length-1; l++)
{
MaxScore = Score[l];
MaxScoreIndex =l;
...
printToLog("----------- Momentum ranking for : " + SelectedAssets.get(i)+ "   --------------");
for(int l=0; l<=Funds.length-1; l++)
    printToLog("Candidate fund " + (l+1) + " : " + Funds[l] + "     Score : " + Score[l]);
...
for(int j = 0; j < RepresentNumber; j++)  
{
    //printToLog("Selected fund " + (j+1) + " : " + Funds[j] + "     Score : " + Score[j]);
    if(!Funds[j].equals("CASH"))
    {
        if(RepresentFundList.indexOf(Funds[j]) == -1)
        {
