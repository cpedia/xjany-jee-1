...
function usercheck(obj){
	...
	if(uservalue==""){
		...
		return false;
   		...
   		document.getElementById(obj).innerHTML="<span class='font_red12'>用户名不能大于15个字符。</span>";
		...
		$.ajax({
			type: "post",
			url: "/user/check.do?userName="+uservalue,
			datatype: "html",
...
function mailcheck(obj){
 	...
 	if(mailvalue==""){
		...
		$.ajax({
			type: "post",
			url: "/user/check.do?userEmail="+mailvalue,
			datatype: "html",
			success: function(data){
				if("" == data || null == data){
