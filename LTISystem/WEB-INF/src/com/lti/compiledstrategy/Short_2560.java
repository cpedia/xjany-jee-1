...
public class Short_2560 extends SimulateStrategy{
	public Short_2560(){
		super();
		StrategyID=560L;
		StrategyClassID=2L;
	...
	private double AdjustBand;
	public void setAdjustBand(double AdjustBand){
		this.AdjustBand=AdjustBand;
	}
	
	public double getAdjustBand(){
		return this.AdjustBand;
	}
	...
	public void fetchParameters()throws ParameterException{
		...
		AdjustBand=(Double)ParameterUtil.fetchParameter("double","AdjustBand", "0.0", parameters);
	}
	//----------------------------------------------------
	//variables
	//----------------------------------------------------
...
CurrentPortfolio.shortSell(curAsset, ShortSecurity1, TotalAmount*ShortPercent1, CurrentDate);
CurrentPortfolio.shortSell(curAsset, ShortSecurity2, TotalAmount*ShortPercent2, CurrentDate);
Shares1=TotalAmount*ShortPercent1/S1.getCurrentPrice(CurrentDate);
Shares2=TotalAmount*ShortPercent2/S2.getCurrentPrice(CurrentDate);
	}
	//----------------------------------------------------
	//re-initialize code
	//----------------------------------------------------
...
Short1=Shares1*getSecurity(ShortSecurity1).getCurrentPrice(CurrentDate);
Short2=Shares2*getSecurity(ShortSecurity2).getCurrentPrice(CurrentDate);
CSPercent1=Short1/(Short1+Short2);
CSPercent2=Short2/(Short1+Short2);
printToLog(CurrentDate + " Short1 "+Short1/(Short1+Short2) + "   Short2 " + Short2/(Short1+Short2));
...
Shares2=TotalAmount*ShortPercent2/S2.getCurrentPrice(CurrentDate);
		...
		else if ((CSPercent1/ShortPercent1 < (1.0-AdjustBand)) || (CSPercent1/ShortPercent1>(1.0+AdjustBand))||(CSPercent2/ShortPercent2 < (1.0-AdjustBand)) || (CSPercent2/ShortPercent2>(1.0+AdjustBand))) {
		   Security S1=getSecurity(ShortSecurity1);
Security S2=getSecurity(ShortSecurity2);
