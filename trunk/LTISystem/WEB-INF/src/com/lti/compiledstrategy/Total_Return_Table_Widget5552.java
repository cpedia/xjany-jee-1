...
public class Total_Return_Table_Widget5552 extends SimulateStrategy{
	public Total_Return_Table_Widget5552(){
		super();
		StrategyID=5552L;
		StrategyClassID=3L;
	}
...
boolean DoLast = false;
	...
	public void writeObject(ObjectOutputStream stream) throws IOException{
		...
		stream.writeObject(DoLast);
	}
	
	public void readObject(ObjectInputStream stream) throws IOException, ClassNotFoundException{
		ToCheck=(Boolean)stream.readObject();;
		DoLast=(Boolean)stream.readObject();;
	}
...
//ccd add it on 2011-03-01
double cashprice = securityManager.getPrice(890L, CurrentDate);
...
String[][] ReturnTable = new String[Funds.length + 1][3];
			int i;
			String SecName;
			Security Sec;


			String FirstRow[] = { "Description", "52 Weeks", "Trend Score" };
			for (i = 0; i < 3; i++) {
...
*/
				ReturnTable[i][1] = String.valueOf(RoundPercent(fiftytwoweek)) + "%";
				ReturnTable[i][2] = String.valueOf(RoundPercent(average)) + "%";
				ReturnTable[i][0] = Descriptions[i - 1] + " (<a target=\"_blank\" href=\"http://www.myplaniq.com/LTISystem/jsp/fundcenter/View.action?symbol=" + SecName + "&type=1&includeHeader=true\">" + SecName + "</a>)" ;
...
/*      if(DoLast){
            ...
            for(int k = 0; k < lastReturnTable.length; k++)
               printToLog("dealing fund : " + lastReturnTable[k][0]);
