...
public class TimeSeriesChartGenerator2 {
	...
	public class MaxMinXYItemLabelGenerator extends AbstractXYItemLabelGenerator implements XYItemLabelGenerator {
		...
		public MaxMinXYItemLabelGenerator(int size, double[] maxes, double[] mins, String[] titles) {
			...
			this.titles = titles;
			TimeSeriesChartGenerator2.this.titles = titles;
		...
		for (int k = 0; k < datas.size(); k++) {
			...
			for (int i = 0; i < values.size(); i++) {
				try {
					...
					if (min > value)
						min = value;
					s1.add(new org.jfree.data.time.Day(date), value);
				} catch (Exception e) {
					e.printStackTrace();
				}
			...
			if(startDate.after(_startDate))startDate=_startDate;
