...
public class Copy_of_Strategic_Asset_Allocation_new_SD_method1012 extends SimulateStrategy{
	public Copy_of_Strategic_Asset_Allocation_new_SD_method1012(){
		super();
		StrategyID=1012L;
		StrategyClassID=3L;
	}
...
for(int i=0; i< CandidateFund.length; i++)
{
	...
	if(LTIDate.before(getEarliestAvaliablePriceDate(getSecurity(CandidateFund[i])), LTIDate. getNewNYSEMonth(CurrentDate, -12)) && LTIDate.before(getEarliestAvaliablePriceDate(Bench), LTIDate. getNewNYSEMonth(CurrentDate, -12))){
		...
		if (assetClass != null) {  //Omit those funds that DO NOT Have ParentClassID
			...
			double BenchSD = BenchMPT.getStandardDeviation();
			SecurityMPT FundMPT = securityManager.getSecurityMPT(Fund.getID(),LastYear);
			double FundSD =FundMPT.getStandardDeviation();
...
for(int i =0; i < Weights.size(); i++)
{
    if(Weights.get(i)!=0){
        ...
        for(int j=0; j< PresentativeFunds.size(); j++)
            CurrentPortfolio.buyAtNextOpen(CurrentPortfolio.getAsset(AvailableAssetClassList.get(i)).getName(), PresentativeFunds.get(j), TotalAmount*Weights.get(i) / PresentativeFunds.size()/100, CurrentDate,true); 
     }
}
printToLog("-------END OF ACTION-------");
	...
	public void action() throws Exception{
		...
		else if (( RebalanceFrequency.equals("monthly") && LTIDate.isLastNYSETradingDayOfMonth(CurrentDate)) 
|| ( RebalanceFrequency.equals("quarterly") && LTIDate.isLastNYSETradingDayOfQuarter(CurrentDate))) {
...
double AvailCapital= 0; /*Available Capital that can be used to buy other funds*/
