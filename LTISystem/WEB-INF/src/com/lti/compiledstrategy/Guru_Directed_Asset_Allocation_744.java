...
public class Guru_Directed_Asset_Allocation_744 extends SimulateStrategy{
	public Guru_Directed_Asset_Allocation_744(){
		super();
		StrategyID=744L;
		StrategyClassID=3L;
	}
...
for(int i = 0; i< WeightDouble.length; i++)
{
    for(int j = 0 ; j< AvailableStableAssetClassList.size(); j++)
        ...
        {
            IsStable[i] = 1;
            printToLog("Stable asset class: "+AvailableStableAssetClassList.get(j));
            printToLog("suggested weight = "+WeightDouble[i] + "  scale factor = " + RiskProfile/StandardRiskProfile);
            SumStable += WeightDouble[i];
            WeightDouble[i] = WeightDouble[i] * (RiskProfile/StandardRiskProfile);
...
printToLog("SumStable = "+ SumStable);
for(int i = 0; i< WeightDouble.length; i++)
{
        if(IsStable[i] == 0)   
        {
            printToLog("Risky asset class: "+ AvailableAssetClassList.get(i));
            printToLog("suggested weight = "+WeightDouble[i] + "  scale factor = " + (1-SumStable*(RiskProfile/StandardRiskProfile)) / (1-SumStable));             
             WeightDouble[i] = WeightDouble[i] * (1-SumStable*(RiskProfile/StandardRiskProfile)) / (1-SumStable); 
             printToLog("adjusted weight = "+WeightDouble[i]);
             break;
        }
}
