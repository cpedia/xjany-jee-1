...
<html>
	...
	<body>
		<div>
			<form action="RecoverPortfolio.action" namespace="/jsp/admin/portfolio" method="post" enctype="multipart/form-data">
				Set End Date: <input type="text" name="endDateStr"/>
		...
		<div>
			<s:form namespace="/jsp/admin/portfolio/Main">
				<s:select id="portfolioSelect" list="#{'0':'ALL','1':'ALIVE','2':'UNALIVE','3':'DEVELOPMENT','4':'LOWLEVEL','5':'EMAILALERT'}" value = "portfolioMode" theme="simple" name="portfolioMode"></s:select>
				<s:submit value="Get Portfolios with Mode" action="Main" theme="simple"></s:submit>
			</s:form>
		</div>
		<table width="100%">
			<tr class="trHeader">
		...
		Change to
			<s:url action="Main.action" id="url_first">
				...
				<s:param name="portfolioMode" value="portfolioMode"></s:param>
			</s:url>
		...
		<form action='<s:property value="%{url_m}"/>' method="post" onSubmit="this.startIndex.value=(this.startIndex.value-1)*this.pageSize.value;return true;">
			...
			<input type="hidden" name="portfolioMode" value="<s:property value="portfolioMode"/>"/>
