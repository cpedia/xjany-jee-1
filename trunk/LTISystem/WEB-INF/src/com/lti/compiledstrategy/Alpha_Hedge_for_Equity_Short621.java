...
public class Alpha_Hedge_for_Equity_Short621 extends SimulateStrategy{
	public Alpha_Hedge_for_Equity_Short621(){
		super();
		StrategyID=621L;
		StrategyClassID=16L;
	}
	...
	private String[] ShortFunds;
	public void setShortFunds(String[] ShortFunds){
		this.ShortFunds=ShortFunds;
	}
	
	public String[] getShortFunds(){
		return this.ShortFunds;
	}
	private String[] LongFunds;
	public void setLongFunds(String[] LongFunds){
		this.LongFunds=LongFunds;
	}
	
	public String[] getLongFunds(){
		return this.LongFunds;
	}
	...
	public void fetchParameters()throws ParameterException{
		...
		Funds=(String[])ParameterUtil.fetchParameter("String[]","Funds", "IWB, IWR, IWM", parameters);
		ShortFunds=(String[])ParameterUtil.fetchParameter("String[]","ShortFunds", "BGZ, MWN, TZA", parameters);
		LongFunds=(String[])ParameterUtil.fetchParameter("String[]","LongFunds", "BGU, MWJ, TNA", parameters);
	}
	//----------------------------------------------------
	//variables
	//----------------------------------------------------
...
//long the highest by shorting the short side fund
String Target;
int j;
Target = list2.get(0).getSymbol();
printToLog("Long Fund: "+Target);
for (j=0; j<Funds.length; j++) {
   if (Target.equals(Funds[j])) break;
}
printToLog("Index j: "+j);
printToLog("ShortFund: "+ShortFunds[j]);
CurrentPortfolio.shortSell(curAsset, ShortFunds[j], amount*0.5 ,CurrentDate);
//short the lowest by shorting the long side fund
Target = list2.get(list2.size()-1).getSymbol();
printToLog("Long Fund: "+Target);
for (j=0; j<Funds.length; j++) {
   if (Target.equals(Funds[j])) break;
}

CurrentPortfolio.shortSell(curAsset, LongFunds[j], amount*0.5, CurrentDate);
