...
public class Market_Timing_Rule_with_Maturity_Spread558 extends SimulateStrategy{
	public Market_Timing_Rule_with_Maturity_Spread558(){
		super();
		StrategyID=558L;
		StrategyClassID=9L;
	}
	...
	public void fetchParameters()throws ParameterException{
		...
		ShortTerm=(String)ParameterUtil.fetchParameter("String","ShortTerm", "^IRX", parameters);
...
Security Bill = getSecurity(ShortTerm);
...
printToLog("The earliest available price date of the bill :"+getEarliestAvaliablePriceDate(Bill));
printToLog("The earliest available price date of the bond :"+getEarliestAvaliablePriceDate(Bond));

if(Threshold<=1)
    ThresholdValue=Threshold;
if(Threshold>1)
   ThresholdValue= (Bond.getSMA(CurrentDate,(int)Threshold,TimeUnit.DAILY)-Bill.getSMA(CurrentDate,(int)Threshold,TimeUnit.DAILY))/100;

Spread= (Bond.getCurrentPrice(CurrentDate) -Bill.getCurrentPrice(CurrentDate))/100;
...
if(DelayDay>=2)
{
    for(int i=2;i<=DelayDay;i++)
    {
        ...
        Spread= (Bond.getCurrentPrice(PastDate) -Bill.getCurrentPrice(PastDate))/100;
        if(Threshold>1)
        {
            PastThresholdValue = (Bond.getSMA(PastDate,(int)Threshold,TimeUnit.DAILY)-Bill.getSMA(PastDate,(int)Threshold,TimeUnit.DAILY))/100;
